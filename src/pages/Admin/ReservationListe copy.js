...
const ReservationListe = () => {
  ...
  useEffect(() => {
    // Filtrage côté client basé sur les critères de recherche
    const filtered = reservations.filter((reservation) => {
      const nomMatch = `${reservation.touriste_prenom} ${reservation.touriste_nom}`
        .toLowerCase()
        .includes(search.nom.toLowerCase());
      const villeMatch = reservation.visite_ville
        .toLowerCase()
        .includes(search.ville.toLowerCase());
      const typeMatch = reservation.visite_type
        .toLowerCase()
        .includes(search.type.toLowerCase());
      return nomMatch && villeMatch && typeMatch;
    });
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      if (!token) {
        setMessage('Vous devez être connecté en tant qu\'administrateur. Redirection vers la connexion...');
        setTimeout(() => navigate('/admin/login'), 2000);
        setLoading(false);
        return;
      }

      console.log('📤 Récupération des réservations à :', `${API_URL}/reservation`);
      ...
      console.log('📥 Réponse de /api/reservation :', res.data);
      setReservations(res.data);
      setFilteredReservations(res.data);
      setLoading(false);
    } catch (err) {
      console.error('❌ Erreur lors de la récupération :', err.response?.data || err.message);
      const errorMessage = err.response?.data?.error || err.response?.data?.message || 'Erreur lors du chargement des réservations.';
      setMessage(errorMessage);
      if (err.response?.status === 401) {
        setMessage('Session expirée. Veuillez vous reconnecter.');
        setTimeout(() => navigate('/Admin/connexionadmin'), 6000);
      }
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.log('📤 Suppression de la réservation à :', `${API_URL}/reservation/${id}`);
      ...
      if (err.response?.status === 401) {
        setMessage('Session expirée. Veuillez vous reconnecter.');
        setTimeout(() => navigate('/Admin/connexionadmin'), 2000);
      }
      setLoading(false);
    }
  };
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    // Le filtrage est géré par useEffect, pas besoin de fetch
  };

  return (
    <div
      className="min-h-screen flex flex-col bg-gray-100 px-4 py-6 transition-opacity duration-700 ease-in-out"
    >
